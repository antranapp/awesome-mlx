...
- [MTPLX](https://github.com/youssofal/MTPLX): Native multi-token prediction inference engine and Mac app for Apple Silicon. Runs Qwen 3.5, 3.6 and 3.8 with their MTP heads, exact at any temperature, up to 125 tok/s on Qwen 3.8 Flash Next on an M5 Max. OpenAI and Anthropic compatible server, works with Claude Code, OpenCode and Continue.
...
- [Chaty](https://github.com/Fangyuan025/Chaty): Local, private desktop AI app (Windows/macOS) with a native MLX backend — a process-isolated Swift sidecar built on mlx-swift-lm — alongside GGUF/llama.cpp. Chat, RAG, vision, voice, and an agentic coding mode, fully offline. (MIT)
- [Logue](https://github.com/bitwize-ai/Logue): Privacy-first AI meeting-notes and writing assistant for macOS. Runs transcription, speaker diarization, and LLM inference on-device via MLX-Swift on Apple Silicon — nothing leaves your Mac. (MIT)
- [Everest](https://github.com/kabrapratik28/Everest): Menu-bar writing assistant for macOS. Select text in any app, press ⌥R, and it is rewritten in place by a local model running on MLX-Swift. No account, no server, no telemetry. (MIT)
